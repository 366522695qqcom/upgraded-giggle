...
import { PolicyType, Policy, Gold } from '../../../core/game/Game';
import { PolicyTechManager } from '../../../core/game/PolicyTechManager';
import { PolicyCategory } from '../../../core/game/PolicyTechSystem';
...
export const PolicyPanel: React.FC<PolicyPanelProps> = ({
  ...
  // 分类国策
  const categorizedPolicies = PolicyTechManager.getAllPolicies().reduce((acc, policy) => {
  ...
  const isPolicyAvailable = (policy: Policy): boolean => {
    return unlockedPolicies.includes(policy.type) && 
  ...
  const PolicyCard: React.FC<{ policy: Policy }> = ({ policy }) => {
    ...
    return (
      <Card className={`p-4 mb-4 border-2 transition-all duration-300 ${active ? 'border-green-500 bg-green-50' : available ? 'border-blue-500 hover:shadow-lg' : 'border-gray-300 opacity-70'}`}>
        <div className="flex justify-between items-start">
          ...
          <div className="text-right">
            ...
            {policy.duration && (
              <div className="text-xs text-gray-500">持续 {policy.duration} 回合</div>
            )}
          </div>
        </div>
        ...
        {/* 显示效果描述 */}
        {policy.effects && Object.entries(policy.effects).map(([key, value]) => {
          ...
          switch(key) {
            case 'goldProductionMultiplier':
              ...
              effectColor = Number(value) > 1 ? 'text-green-600' : 'text-red-600';
              break;
          ...
          return (
            <div key={key} className={`text-sm font-medium ${effectColor} mb-1`}>
              • {effectDescription}
            </div>
          );
        })}
        ...
        <div className="mt-4 flex justify-between items-center">
          <div className="text-xs text-gray-500">
            ...
            {!active && isUnlocked && hasEnoughGold && !canEnactPolicy(policy.type) && (
  ...
  return (
    <div className="h-full flex flex-col bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
      <div className="p-4 border-b border-gray-200 bg-gray-50">
        <h2 className="text-2xl font-bold text-center text-gray-800">国策管理</h2>
          ...
          <TabsList className="w-full">
            ...
            {categories.map(category => (
              <TabsTrigger 
                ...
                className="flex-1 data-[state=active]:bg-blue-100 data-[state=active]:text-blue-800"
              >
                {category === 'economic' ? '经济' :
                 category === 'military' ? '军事' :
                 category === 'cultural' ? '文化' :
                 category === 'diplomatic' ? '外交' :
                 category === 'infrastructure' ? '基建' : category}
              </TabsTrigger>
            ))}
          </TabsList>
        </div>
