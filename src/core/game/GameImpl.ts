...
export class GameImpl implements Game {
  ...
  handlePlayerMessage(player: Player, message: Message): void {
    switch (message.type) {
      case MessageType.MOVE_UNIT:
        // 处理移动单位消息
        break;
      case MessageType.ATTACK:
        // 处理攻击消息
        break;
      case MessageType.TRAIN_UNIT:
        // 处理训练单位消息
        break;
      case MessageType.BUILD_CITY:
        // 处理建造城市消息
        break;
      case MessageType.UPGRADE_CITY:
        // 处理升级城市消息
        break;
      case MessageType.TRADE_RESOURCES:
        // 处理资源交易消息
        break;
      case MessageType.DECLARE_WAR:
        // 处理宣战消息
        break;
      case MessageType.SEND_DIPLOMATIC_MESSAGE:
        // 处理外交消息
        break;
      case MessageType.ENACT_POLICY:
        this.handleEnactPolicy(player, message.data);
        break;
      case MessageType.START_RESEARCH:
        this.handleStartResearch(player, message.data);
        break;
      default:
    ...
    if (playerImpl) {
      ...
      if (success) {
        ...
        this.broadcastGameMessage({
          type: MessageType.POLICY_ENACTED,
          senderId: player.id(),
          data: {
  ...
  private handleStartResearch(player: Player, data: StartResearchMessageData): void {
    ...
    if (playerImpl) {
      ...
      if (success) {
        // 广播研究开始的消息
        this.broadcastGameMessage({
          type: MessageType.RESEARCH_STARTED,
          senderId: player.id(),
          data: {
