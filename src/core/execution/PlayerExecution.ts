...
export class PlayerExecution implements Execution {
  ...
  tick(ticks: number) {
    ...
    // 使用考虑国策和科技加成的有效军队训练率
    const effectiveTroopRate = this.player.getEffectiveTroopTrainingRate();
    this.player.addTroops(effectiveTroopRate);
    // 使用考虑国策和科技加成的有效金币生产率
    const effectiveGoldRate = this.player.getEffectiveGoldProductionRate();
